...
- **Verified live release:** root confirmed `553639b7f7982eaebc4114d84c0cbc1496370d8a`, tree `f4670a0860fc1ae81dfafeed5e9584b2cc610460`, at https://www.browserp.com through `dpl_BR4v6yNXZE9ZYC6yHMCbrrcpDn2a`. The integrated gate passed **936 checks** and **12 hosted smoke checks**. Local `4257d5d` had the same tree before colour patch `e72d7749`; later local colour/visual work is not included in that production proof. Old previews `4c985` and `9r07e9pw4` are stale restart evidence, not the current live release.
- **Live discovery and staff boundaries:** the expanded Discover menu, default **63** results and root-click behaviour were verified live. The staff-profile dropdown uses the protected gradient action. Anonymous staff documents return **401** with a generic response and `private, no-store` caching; current session, enabled staff policy and verified TOTP remain required, with RPC checks authoritative. This does not establish Discord-to-website staff synchronisation.
- **Website defects still open:** the double-dim hero wave, weak header pulse, oversized All games bar and advert crop/clipping are with the homepage agent. Preserve the established branding, artwork, controls and behaviour while correcting them. Finish actual visual review, cohesion, Chromium/Firefox/WebKit, mobile/desktop, slower-network/lag and reduced-motion checks against the corrected release; the preceding test gate does not close these observed defects.
...
**Website → Discord role synchronisation is newly agreed and not implemented.** The website must remain the source of truth, using owner-approved exact Discord role IDs, active staff membership **and** enabled allowlist policy, and a confirmed Discord identity. Removal or suspension must revoke the mapped Discord role. Never infer Ownership from a role name, ordinary Discord membership or a display label; prevent accidental escalation. Define and verify these boundaries before enabling any sync. Discord Member verification must not grant website staff access.
