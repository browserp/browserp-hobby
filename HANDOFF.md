...
This section supersedes the historical branch, pending-work and usage snapshots below. Continue on **`release/seo-community-home-20260906`**. Preserve **at least 50% weekly allowance remaining**; root's latest snapshot was **31% used / 69% remaining**. Finish agreed work without expanded research or new tasks, and make **one coordinated release** for this batch instead of periodic publications. The removed-scope decisions below still apply.

- **Release:** root last reported source `fb9090c1b1270f4a8b71b63adb86c6fe6af5360a` READY in preview `9r07e9pw4`, **not yet production**. Root handles promotion after final verification. Do not treat this preview as containing later working changes or redeploy solely for these notes.
- **Website/staff work:** the five Discover controls are committed in `1bb70d5`, with 28 focused tests, six browser cases and static checks reported passing. Staff work remains pending: a profile-dropdown Staff panel action using the Search gradient and strict server-confirmed staff visibility; workspace HTML gates for canonical/nested/flat/`.html` routes using current Discord session, live staff assignment/allowlist policy and verified TOTP. Denied visitors should receive a generic sign-in shell; RPC permissions remain authoritative. Staff implementation, focused tests, secure-auth documentation and the final combined release check are pending. No new provider/database changes or functions are part of that staff work.
- **Discord privacy:** the imported public exception is resolved. `staff-chat` (`1546229400016527370`) and all four Staff text channels are private; Staff voice is synced private. Owner office has explicit View Channels/Connect/Speak denies and is intentionally unsynced after its Speak override. Staff roles are still unassigned and not allowed into Staff; the actual owner and Security currently have access. Six named staff roles have dangerous native permissions OFF, but hierarchy, appropriate access and restricted bot commands remain pending.
- **Discord counters and cleanup:** installed bots are Security and ServerStats; Xenon is removed. ServerStats voice/Default setup at 8:26 PM read **All Members 3 / Members 1 / Bots 2**, with the bot stating **15-minute updates**. Boost counting remains unconfigured. All **45 unused template roles plus Special Forces (46 total)** are deleted, with **18 retained roles** verified. Support Team, Announcements, Creator, Server Owner and Member have extra native permissions disabled and public mentions off; four game roles and First 100 retain prior restricted settings. Human role names are emoji-free; hierarchy/access work is still pending. Security is installed but unconfigured. Emoji uploads are complete (40, with 10/50 static slots remaining); attribution is retained in `browserp-platform/docs/discord-assets/`.

Finish the agreed Discord setup and ordinary-member checks without claiming global privacy, live booster statistics or proven security. No public invite has been published. Details and remaining work are in `browserp-platform/docs/DISCORD_COMMUNITY_SETUP_2026-09-06.md` and the current section of `LAUNCH_CONTINUITY.md`. Keep recovery codes and credentials out of notes; the owner's deferred recovery-code replacement stays deferred.
