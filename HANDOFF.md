...
**Final onboarding readback:** native Onboarding/Server Guide is enabled with four default channels and four optional cosmetic game roles, all mappings verified. The owner selected all four roles, confirmed persistence after reload, then removed all four and confirmed the restored state after another reload. Unverified View/Read History access to public rules was saved and reopened with Send denied; verification access was already correct. This establishes enabled configuration and owner-tested role selection/removal, not a new ordinary member’s joining/verification journey or requester privacy. The approximate completion estimate remains unchanged.
