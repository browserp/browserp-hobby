...
const providerQr = '<?xml version="1.0"?>\n<!-- Generated by SVGo -->\n<svg xmlns="http://www.w3.org/2000/svg"></svg>';
...
async function fixture(run, options = {}) {
  ...
  globalThis.fetch = async (value, config = {}) => {
    ...
    if (path === "/auth/v1/factors") { state.factors.push(factor(pending, "unverified", body.friendly_name)); return response({ id: pending, totp: { secret: "ABCDEFGHIJKLMNOP", qr_code: providerQr } }); }
...
test("legacy enrollment cannot bypass existing staff verification, factor capacity or concurrent-operation guards", async () => fixture(async state => {
  ...
  assert.equal(Buffer.from(success.payload.factor.qrCode.split(",")[1], "base64").toString(), '<svg xmlns="http://www.w3.org/2000/svg"></svg>');
}));
...
test("backup setup uses BrowseRP issuer, stays unverified until a code is supplied, rotates session and audits without secrets", async () => fixture(async state => {
  ...
  assert.equal(Buffer.from(enrolled.setup.qrCode.split(",")[1], "base64").toString(), '<svg xmlns="http://www.w3.org/2000/svg"></svg>');
