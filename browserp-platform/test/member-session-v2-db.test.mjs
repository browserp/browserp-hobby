...
test("real private file export consumers retain session and ownership boundaries after legacy retirement", async t => {
  const {db, a, b, owner, sid, admin, login, call, create, review, approve, generate} = await privacyFileFixture(t);
  const assetId = randomUUID(), sha = "a".repeat(64);
  await admin(`insert into public.uploaded_assets(id,owner_id,bucket,object_path,media_type,mime_type,byte_size,sha256)
    values('${assetId}','${a}','profile-media','${a}/retirement-fixture.png','avatar','image/png',100,'${sha}');
    insert into storage.objects(bucket_id,name,metadata) values('profile-media','${a}/retirement-fixture.png','{"size":100}');`);
  const before = (await db.query("select md5(prosrc) body from pg_proc where oid='public.member_connection_status()'::regprocedure")).rows[0].body;
  await db.exec(v2Sql);
  await db.exec(retirementSql);
  const after = (await db.query("select md5(prosrc) body from pg_proc where oid='public.member_connection_status()'::regprocedure")).rows[0].body;
  assert.equal(after, before, "retirement must preserve the actual guarded legacy body for internal callers");

  // Use the real request, staff approval, export preparation and file functions,
  // not an export helper stub. All records and Storage metadata are synthetic.
  ...
  await t.test("an approved member can prepare and read its exact file while direct legacy execution stays denied", async () => {
    const descriptor = await readFile();
    assert.equal(descriptor.assetId, assetId); assert.equal(descriptor.sha256, sha);
    assert.equal(descriptor.objectPath, `${a}/retirement-fixture.png`);
    await assert.rejects(call("member_connection_status"), {code:"42501"});
    assert.equal((await call("member_connection_status_v2")).active, true);
    const receipt = await call("member_receive_data_export_file", [copy.id, file.id, sha, true]);
    assert.ok(receipt.receivedAt);
  });
  await t.test("other members, staff, anonymous and service callers cannot obtain the member file", async () => {
    for (const user of [b, owner]) { await login(user); await assert.rejects(readFile(), /not found/); }
    for (const role of ["anon","service_role"]) { await admin(`set role ${role}`); await assert.rejects(readFile(), {code:"42501"}); }
  });
  await t.test("stale OAuth, revoked sessions and withdrawal still deny the real file consumer", async () => {
    await login(a, {amr:[{method:"oauth",timestamp:Math.floor(Date.now()/1000)-601}]});
    await assert.rejects(readFile(), /Sign in again/);
    await admin(`delete from auth.sessions where id='${sid(a)}'`); await login();
    await assert.rejects(readFile(), /active, unrestricted/);
    await admin(`insert into auth.sessions(id,user_id) values('${sid(a)}','${a}')`); await login();
    assert.equal((await readFile()).id, file.id);
    await call("member_data_requests", ["withdraw",null,null,null,row.id,row.version]);
    await assert.rejects(readFile(), /request changed/);
  });
});
