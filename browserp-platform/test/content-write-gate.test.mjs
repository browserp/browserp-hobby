...
const commentId = "cccccccc-0000-4000-8000-000000000002";
const submissionId = "dddddddd-0000-4000-8000-000000000001";
...
async function fixture(run, paused = "true") {
  const values = { CONTENT_WRITES_PAUSED: paused, SUPABASE_URL: "https://fixture.supabase.co", SUPABASE_PUBLISHABLE_KEY: "sb_publishable_fixture",
    ...
    CONTENT_MODERATION_PROVIDER: "", OPENAI_API_KEY: "" };
  ...
  globalThis.fetch = async (value, options = {}) => {
    ...
    calls.push({ origin: url.origin, path: url.pathname, body, options });
    ...
    if (url.pathname.endsWith("/member_server_interaction")) {
      if (body.p_action !== "comment") return response({ accepted: true });
      moderationItem = { id: submissionId, kind: "comment", targetId: commentId, text: body.p_body, status: "pending_review", version: 1,
        appealStatus: "none", reason: "Waiting for staff review." };
      return response({ id: commentId, status: "pending_review" });
    }
    if (url.pathname.endsWith("/member_content_moderation_item")) return response(moderationItem);
    if (url.pathname.endsWith("/service_content_check_input")) return response({ ...moderationItem, ownerId: member, fingerprint: "a".repeat(64), duplicate: false });
    if (url.pathname.endsWith("/service_record_content_check")) return response(true);
    if (url.pathname.endsWith("/member_apply_content_check")) return response(moderationItem);
...
test("reopening this deployment restores normal comment handling and avatar validation", async () => fixture(async calls => {
  ...
  assert.equal(comment.body.moderation.id, submissionId);
  assert.equal(comment.body.moderation.status, "pending_review");
  const recorded = calls.filter(call => call.path.endsWith("/service_record_content_check"));
  assert.equal(recorded.length, 1);
  assert.equal(recorded[0].body.p_result.decision, "review");
  assert.equal(recorded[0].body.p_result.details.code, "provider_not_enabled");
  const applied = calls.filter(call => call.path.endsWith("/member_apply_content_check"));
  assert.equal(applied.length, 1);
  assert.deepEqual(applied[0].body, { p_id: submissionId, p_version: 1 });
  assert.equal(applied[0].options.headers.Authorization, `Bearer ${token}`);
  ...
  assert.ok(calls.every(call => call.origin === "https://fixture.supabase.co"));
}, "false"));
