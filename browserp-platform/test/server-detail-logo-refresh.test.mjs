...
test("server identity stays in one compact heading while details use the full card width", () => {
  ...
  try {
    ...
    assert.ok(title.querySelector("#server-website-v3"), "The official website action must remain available");
...
test("live FiveM refresh stores validated artwork and returns it immediately", () => {
  ...
  assert.match(workflow, /website_url: info\.websiteUrl \?\? server\.website_url \?\? null/);
