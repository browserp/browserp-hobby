...
test("public staff projection exposes active roster fields only and orders by rank", () => {
  ...
  for (const member of staff) {
    assert.equal("userId" in member, false);
    assert.equal("roleKey" in member, false);
    assert.equal("rank" in member, false);
    assert.equal("status" in member, false);
    assert.equal("permissions" in member, false);
  }
});
...
test("public roster uses only approved, allowlisted avatar locations", () => {
  ...
  for (const url of [
    "http://cdn.discordapp.com/avatars/123456789012345678/hash.png",
    "https://evil.example/avatar.png",
    "https://kywabzfgjoqiznnxygbq.supabase.co/storage/v1/object/public/advertisements/x.png",
    "https://user:password@cdn.discordapp.com/avatars/123456789012345678/hash.png"
  ]) assert.equal(safePublicStaffAvatar(url), null, url);

  const hidden = publicStaffView(
    memberships,
    roles,
    profiles.map((profile) => profile.id === adminId ? { ...profile, avatar_review_status: "pending_review" } : profile)
  );
...
test("staff page is a clean public page with current roster states and no private controls", () => {
  ...
  assert.doesNotMatch(page, /Discord ID|MFA|permission overrides|email address/i);
...
test("existing resources function multiplexes staff without creating a thirteenth Vercel function", () => {
  ...
  assert.equal(vercel.functions["api/router.js"].maxDuration, 60);
});
