...
async function fixture(run,options={}){
  ...
  globalThis.fetch=async(value,config={})=>{
    ...
    if(call.path==="/auth/v1/factors"){state.factors.push(factor(replacement));return response({id:replacement,totp:{secret:"ABCDEFGHIJKLMNOP",qr_code:providerQr}});}
...
test("the public routes use the initial lock for restart and verification without serializing provider sessions",async()=>{
  ...
  await fixture(async state=>{
    ...
    assert.equal(Buffer.from(restarted.payload.factor.qrCode.split(",")[1],"base64").toString(), '<svg xmlns="http://www.w3.org/2000/svg"></svg>');
