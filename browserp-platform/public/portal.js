...
async function serverPage(session) {
  ...
  try {
    ...
    root.innerHTML = `<section class="server-profile-hero"><div class="shell server-profile-head"><div class="server-profile-logo">${escapeHtml(initials(server.name))}</div><div><span class="section-kicker server-kicker">FiveM · ${escapeHtml(server.region)}</span><h1>${escapeHtml(server.name)}</h1><p>${server.online ? "Online now" : "Status unavailable"} · ${escapeHtml(server.language)} · ${escapeHtml(server.framework || "Server setup not listed")}</p><div class="server-profile-tags">${(server.tags || []).slice(0, 6).map((tag) => `<span>${escapeHtml(tag)}</span>`).join("")}</div></div><div class="server-live"><strong>${playerCount}</strong><small>${server.online ? `of ${capacity} players` : "player count unavailable"}</small></div></div></section><main class="shell server-detail-grid"><article class="detail-card"><span class="section-kicker">ABOUT THIS SERVER</span><h2>What to expect</h2><p>${escapeHtml(server.description)}</p>${action}</article><aside class="detail-card"><h2>Server information</h2><div class="signal-list"><div class="signal-row"><span>Listing owner</span><strong>${server.verified ? "Verified" : "Not verified"}</strong></div><div class="signal-row"><span>30-day uptime</span><strong>${uptime}</strong></div><div class="signal-row"><span>Beginner friendly</span><strong>${server.beginner_friendly ? "Yes" : "Not marked"}</strong></div><div class="signal-row"><span>Region</span><strong>${escapeHtml(server.region)}</strong></div></div><p class="server-verification-note">Owner verification confirms control of this listing. It is not a guarantee of server quality.</p></aside></main>`;
  } catch (error) {
