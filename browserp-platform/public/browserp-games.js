(() => {
  ...
  const FIVEM_SHOWCASE = Object.freeze({ slug: "san-andreas-county-roleplay-showcase", showcase_url: "/server/san-andreas-county-roleplay-showcase", name: "San Andreas County Roleplay", platform_name: "FiveM", region: "United States", framework: "vMenu", description: "A complete BrowseRP showcase for a public-safety focused county community.", logo_url: "/assets/san-andreas-county-rp-mark-v4.svg", showcase: true });
  ...
  function gameCard(game) {
    const link = node("a", "game-hub-card-v4"); link.href = `/games/${game.id}`;
  ...
  function serverCard(server) {
    ...
    link.append(top, node("h3", "", server.name || "Roleplay server"), node("div", "server-meta", [server.platform_name, server.region, server.framework].filter(Boolean).join(" · ")), node("p", "server-description", server.description || "Open the listing to learn more."));
  ...
  function render() {
    ...
    nav.append(...GAMES.map((item) => { const link = node("a", "game-nav-chip-v4", item.name); link.href = `/games/${item.id}`; link.dataset.game = item.id; link.prepend(icon(item.id, "game-nav-mark-v4")); if (game?.id === item.id) { link.classList.add("is-selected"); link.setAttribute("aria-current", "page"); } return link; }));
    if (!game) {
