(() => {
  ...
  async function loadArticle() {
    ...
    try {
      ...
      if (!unavailable && root.dataset.publicRendered === "true") {
        status.textContent = "Updates are unavailable. You can still read this article or try again.";
        retry.hidden = false; status.dataset.error = "true"; return;
      }
      if (unavailable) { root.replaceChildren(); for (const selector of ["#journal-excerpt-v6", "#journal-date-v6", "#journal-reading-v6"]) $(selector).textContent = ""; }
