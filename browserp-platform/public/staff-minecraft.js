(() => {
  ...
  function parseInputs(value) {
    ...
    if (!inputs.length || inputs.length > 10) throw new Error("Enter between 1 and 10 Minecraft server addresses.");
    return inputs;
  }
  ...
  async function init({ api, root, imageUrl } = {}) {
    ...
    const fetchForm = make("form"); const sourceLabel = make("label", undefined, "field-v3"); const inputs = make("textarea"); inputs.name = "inputs"; inputs.rows = 3; inputs.maxLength = 3000; inputs.required = true; inputs.placeholder = "play.example.com\nplay.example.net:25565";
    sourceLabel.append(make("span", "Minecraft server addresses"), inputs, make("small", "Up to 10, separated by spaces, commas or new lines.", "fivem-help"));
    const fetchActions=make("div",undefined,"fivem-actions"),fetchButton=button("Fetch for review",true);fetchButton.type="submit";
    const editionLabel=make("label",undefined,"field-v3"),edition=make("select");edition.name="edition";editionLabel.append(make("span","Minecraft edition"),edition);
    for(const[value,label]of[["java","Java Edition"],["bedrock","Bedrock Edition"]]){const option=make("option",label);option.value=value;edition.append(option);}
    fetchActions.append(fetchButton);fetchForm.append(editionLabel,sourceLabel,fetchActions);
    ...
    field("framework", "Roleplay mode", { required: false, maxLength: 80, placeholder: "Unknown", help: "Leave blank if the source does not establish a framework." });
    ...
    field("discordUrl", "Discord community URL", { type: "url", required: false, maxLength: 1000, help: `A Discord invite for the community. This is separate from the Minecraft server address.` });
    ...
    function renderRecords() {
      ...
      records.replaceChildren(...selected.map((item) => {
        ...
        copy.append(make("strong", candidate.name || "Name not provided"), make("span", `${candidate.address || "Unknown address"} · ${candidate.edition || "java"} · Network count`, "fivem-help"), make("small", `Source checked ${date(candidate.checkedAt)} · ${list(candidate.warnings).length} source warnings`, "fivem-help"));
    ...
    async function open(item) {
      ...
      join.replaceChildren(make("strong", "Minecraft address: "),make("code",candidate.address||"Unknown"),make("span",` · ${candidate.edition||"java"} · Count covers the advertised server/network, not one RP world.`));
    ...
    async function mutate(action) {
      ...
      try {
        ...
        message(status, action === "publish" ? response.message || "Reviewed listing published. Open it again to review any further changes." : "Import dismissed.");
    ...
    fetchForm.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        for (let index = 0; index < values.length; index += 1) {
          ...
          try { const payload = await api(endpoint, { method: "POST", body: JSON.stringify({ action: "fetch", inputs: [values[index]], edition: edition.value }) }); count += list(payload.candidates).length; failed.push(...list(payload.errors)); }
  ...
  window.BrowseRPStaffMinecraft = Object.freeze({ init, parseInputs, secureUrl });
})();
