(() => {
  ...
  function mount({ root, list, empty, count, render, fixedGame }) {
    let filters = read(fixedGame), facets = {}, nextOffset = null, requestId = 0, controller, timer, shown = [], retryAppend = false;
    ...
    const selects = {};
    for (const key of ["platform", "region", "mode", "feature", "access", "language"]) {
    ...
    refinements.append(checks); root.append(primary, refinements);
    ...
    function invalidate() { requestId++; controller?.abort(); clearTimeout(timer); }
    ...
    async function load(append) {
      ...
      list.setAttribute("aria-busy", "true"); count.textContent = "Updating servers…"; more.disabled = true; retry.hidden = true;
      const params = M.params(snapshot); params.set("discover", "true"); if (params.has("q")) { params.set("query", params.get("q")); params.delete("q"); }
      try {
        const response = await fetch(`/api/servers?${params}`, { headers: { Accept: "application/json" }, credentials: "same-origin", signal: controller.signal });
        if (!response.ok) throw new Error("Directory unavailable");
        const payload = await response.json(); if (id !== requestId) return;
        facets = payload.facets || {}; nextOffset = payload.nextOffset ?? null;
        shown = append ? [...shown, ...payload.servers] : payload.servers;
        shown = [...new Map(shown.map(server => [server.slug, server])).values()];
        render(list, shown); list.hidden = !shown.length; empty.hidden = Boolean(shown.length);
