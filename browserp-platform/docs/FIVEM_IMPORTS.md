...
Review the name, description, community language, region, framework, access, tags, keywords and images. Follow the source evidence when fields are missing or disagree. Locale suggests a community language/region; it does not prove a server's physical location. The join link is fixed to the source code. Discord invites and image URLs are classified by their destinations. Unsupported media must be removed or replaced. Publishing copies supported raster images to BrowseRP storage and records the reviewer and reason.
