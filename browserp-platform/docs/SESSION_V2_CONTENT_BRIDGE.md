...
The separate disposable PostgreSQL suite passed 18 tests with zero failures. It verifies v1/v2 contract equivalence, active/revoked/foreign/expired/banned/deleted/anonymous access, OAuth recency, staff status, grants before and after retirement, and the actual internal connection-operation and recent-export helpers after retirement. The private recent-export helper is invoked through a disposable postgres-owned adapter. Full hosted file delivery is not exercised. This is synthetic local database evidence, not a hosted migration or full-chain deployment proof.
