...
## Current ticket restoration and verification — 7 September 2026, after 02:00 local

**Ticket Tool is restored, and Help, Reports and Appeals are published in help-desk.** This supersedes the outage, hCaptcha blocker and unpublished-panel statements retained as history below. The root task repaired the replacement managed role's access in all three private ticket categories, ticket-transcripts and help-desk; the entry channel has the bot's View, Send, Embed, Attach and History allows saved. The private greetings and concise panel text were checked in actual tickets. These are owner-controlled checks, not ordinary-member isolation proof.

| Panel | Published message | Controlled ticket and result |
| --- | --- | --- |
...
| Reports | `1546321871929090049` | Ticket `1546322069409759264` opened at 01:51, closed as closed-0001 and was manually archived; confirmation message `1546322719237349380`. Support Team and Trial Moderator are excluded from report access. |
...
All three **create → close → manual archive** flows passed without another bot kick. Archives belong in private **ticket-transcripts** `1546280076612730991`; the public entry channel remains **help-desk** `1546229406169825310`. The archive's actual access was verified as **Ownership, Management, Ticket Tool, Security and the owner only**, excluding Support Team, Moderator and Admin. Help includes Support Team, while Reports and Appeals use the narrower staff access described above. These owner-controlled flows and native access readbacks do not establish isolation between two ordinary requesters or every staff command's effective permission.

**Deletion and capacity settings saved:** global Delete has **Enabled = 0**, verified after reloading. Each of Help, Reports and Appeals has **Delete button Show = 0**, confirmed by the closed-ticket controls. Help's fresh settings read **Two Step Close = 1 / Two Step Ticket = 1**; all three tested closes retained their channels. Closed tickets are retained for manual transcripts and the actual owner's native Discord purge. Each panel now has **Ticket Limit = 1** per user and **Open tickets / all users = 40**, saved and read back on all three. An earlier 40 entry had not persisted and Appeals still showed the real default 500 until this correction. These simultaneous-ticket limits do not impose a ten-minute creation budget.

**Creation-capacity integration remains pending.** Security currently permits five channel creations per actor in ten minutes before the next action triggers its punishment; normal ticket demand can exceed that. Ticket Tool's documented free throttle is one ticket per ten seconds per user and ten tickets per ten seconds per server, which does not resolve that mismatch. Its Two Step Ticket option must remain enabled: disabling it makes Close delete immediately. The bounded supported option under review is a **Ticket Tool user exception for Channel Create only**, retaining Channel Delete and all other action protections; no such exception is claimed saved here. Security documents neither parent-category inheritance for this purpose nor a combined actor-and-category condition. Keep all-panel Delete restrictions and owner-only native purge; do not run another bot-deletion test. Sources: [Ticket Tool free rate limit](https://docs.tickettool.xyz/debugging/errors/4f112fae-f4d3-11ed-a05b-0242ac120003), [close behaviour](https://docs.tickettool.xyz/dashboard/panel-configs/general-options), [Security action whitelists](https://docs.securitybot.gg/whitelist/whitelist).
...
**Remaining bounded work:** settle and read back the creation-only integration; check ordinary-member verification, requester/transcript isolation and effective staff command access; finish channel/Bots-role cleanup and the existing community-content workflow. The user's added scope includes a functional **AFK voice channel**, distinct **bug, member and server reports**, **ban appeals** and **staff applications**. The prepared six-panel plan retains Help, adapts Reports to Member Reports, keeps Appeals, and adds Bug Reports with Help staff, Server Reports with the moderation team, and Staff Applications restricted to Ownership/Management. Three-question forms are prepared with two required answers and one optional answer, with no real-name/date-of-birth collection or automatic staff assignment. Creation and form submission for these added panels are not yet verified here. An in-server appeal panel cannot serve someone already banned from the guild; the website's existing `/appeal` requires a platform BRP reference and is not an established Discord-ban intake. Moderator 2FA, music playback/member controls, moderated ideas and role-sync activation retain their earlier open status unless a later verified checkpoint supersedes them. Discord is not recorded complete.
