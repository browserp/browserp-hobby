...
export function publicStaffView(memberships, roles, profiles) {
  if (!Array.isArray(memberships) || !Array.isArray(roles) || !Array.isArray(profiles)) {
  ...
  return memberships
    ...
    .flatMap((membership) => {
      ...
      const avatarUrl = profile.avatar_review_status === "approved"
        ...
        : null;
      return [{ displayName, roleName: role.name, joinedAt, avatarUrl, _rank: role.rank }];
    })
...
export async function publicStaffRoster() {
  ...
  return publicStaffView(memberships, roles, profiles);
}
