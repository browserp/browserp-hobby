...
export async function publicMemberByUsername(username, req, res) {
  ...
  const submissions = list(await rest(`server_submissions?select=id&submitted_by=eq.${row.id}&limit=80`, { useSecret: true }));
  const ids = submissions.map(s => s.id).filter(id => UUID.test(id));
  const [badges, servers] = await Promise.all([
    rpc("service_member_badges", { p_user_id: row.id }, undefined, { useSecret: true }),
    ids.length ? rest(`servers?select=slug,name,description,region,language,framework,access_type,platform_id,logo_url,banner_url,tags,status,age_rating,verified&source_submission_id=${inIds(ids)}&status=eq.published&limit=24`, { useSecret: true }) : []
  ]);
