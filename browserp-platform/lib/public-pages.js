...
function categoryHref(server, index) {
  const raw = index === 0 ? server.platform_id : index === 4 ? server.access_type : entries(server)[index][1];
  if (!raw || String(raw).trim().toLowerCase() === "unknown" || (index === 0 && server.platform_id === "other")) return "";
  const query = new URLSearchParams();
  if (index > 2 && server.platform_id && server.platform_id !== "other") query.set("platform", server.platform_id);
  query.set(categoryKeys[index], String(raw));
  return `/servers?${query}`;
}
function categoryHTML(server, index, label, value, className) {
  const href = categoryHref(server, index);
  if (!href) return "";
  return `<a class="${className}" href="${escapeHTML(href)}" aria-label="${escapeHTML(`Browse servers filtered by ${label}: ${value}`)}">${index ? escapeHTML(value) : badge(server)}</a>`;
}
function metadata(server, linked = false) { return `<div class="server-meta platform-meta-v5">${entries(server).map(([label, value], i) => value ? (linked ? categoryHTML(server, i, label, value, `server-category-link-v10${i ? " metadata-value-v5" : " game-category-link-v10"}`) : "") || (i ? `<span class="metadata-value-v5" aria-label="${escapeHTML(`${label}: ${value}`)}">${escapeHTML(value)}</span>` : badge(server)) : "").join("")}${server.verified ? '<span class="metadata-value-v5">Owner verified</span>' : ""}</div>`; }
function facts(server) { return `<dl class="server-info-grid-v5" data-platform="${server.platform_id}">${[...entries(server), ["Player status", server.applicationOnly ? "Live player count not provided" : "Check live status on this page"]].map(([label, value], i) => `<div class="server-info-card-v5${i >= 4 ? " server-info-wide-v5" : ""}"><dt>${label}</dt><dd>${i < 5 && value ? categoryHTML(server, i, label, value, "server-fact-link-v10") || (i ? escapeHTML(value) : badge(server)) : i ? escapeHTML(value || "Not specified") : badge(server)}</dd></div>`).join("")}</dl>`; }
  ...
  return async function publicPage(req, res) {
    ...
    try {
      ...
      if (path.startsWith("/server/")) {
        ...
        for (const [id, value] of [["server-name-v3", escapeHTML(server.name)], ["server-description-v3", escapeHTML(server.description)], ["server-platform-v3", `${server.platform_name} roleplay listing`], ["server-meta-v3", metadata(server, true)], ["server-info-v5", facts(server)], ["server-initials-v3", server.logo_url ? `<img class="server-import-logo-v3" src="${escapeHTML(server.logo_url)}" alt="" width="96" height="96">` : initials(server)], ["server-status-v3", server.applicationOnly ? "Live player count not provided" : "Check live status on this page"], ["server-tags-v3", server.tags.filter(tag => tag.trim()).map(tag => `<a class="tag-v3 server-tag-link-v8" href="${escapeHTML(`/servers?${new URLSearchParams({ platform: server.platform_id, feature: tag })}`)}" aria-label="${escapeHTML(`Browse ${server.platform_name} servers tagged ${tag}`)}">${escapeHTML(tag)}</a>`).join("")]]) html = slot(html, id, value);
