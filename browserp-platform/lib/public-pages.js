...
function card(server) {
  ...
  return `<a class="server-card" href="/server/${server.slug}" data-platform="${server.platform_id}"><div class="server-card-top"><div class="server-card-media">${image ? `<img class="server-card-media-image" src="${escapeHTML(image)}" alt="" loading="lazy" width="96" height="96">` : `<span class="server-initials">${initials(server)}</span>`}</div><span class="status">${server.applicationOnly ? "Reviewed community" : "Reviewed listing"}</span></div><h3>${escapeHTML(server.name)}</h3>${metadata(server)}<p class="server-description">${escapeHTML(server.description)}</p><div class="server-card-bottom"><strong>${server.applicationOnly ? "Live player count not provided" : "Check live status"}</strong><span class="server-card-action">View listing</span></div></a>`;
}
...
export function articleBody(body) { return String(body).replace(/\r\n?/g, "\n").split(/\n{2,}/).map(block => { const value = block.trim(); if (!value) return ""; const heading = value.match(/^(#{1,3})\s+(.+)$/); if (heading) { const tag = heading[1].length === 3 ? "h3" : "h2"; return `<${tag}>${escapeHTML(heading[2])}</${tag}>`; } if (value.split("\n").every(line => /^[-*]\s+/.test(line))) return `<ul>${value.split("\n").map(line => `<li>${escapeHTML(line.replace(/^[-*]\s+/, ""))}</li>`).join("")}</ul>`; return `<p>${escapeHTML(value.replace(/\n/g, " "))}</p>`; }).join(""); }
