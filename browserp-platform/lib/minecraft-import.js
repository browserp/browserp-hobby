...
const timeout = (promise,ms) => { let timer; return Promise.race([promise,new Promise((_,reject)=>{timer=setTimeout(()=>reject(error("The server address lookup timed out.")),ms);})]).finally(()=>clearTimeout(timer)); };
export async function minecraftDestination(input,{resolve4Impl=resolve4,resolve6Impl=resolve6,resolveSrvImpl=resolveSrv}={}) {
  let host=input.host,port=input.port;
  if(input.edition==="java"&&input.port===25565&&!isIP(host)) {
    let records=[];
    try {records=await timeout(resolveSrvImpl(`_minecraft._tcp.${host}`),2500);} catch(e) {if(!["ENODATA","ENOTFOUND"].includes(e.code))throw e;}
  ...
  if(!addresses.length){const answers=await timeout(Promise.allSettled([resolve4Impl(host),resolve6Impl(host)]),2500);addresses=answers.flatMap(r=>r.status==="fulfilled"?r.value:[]);}
...
async function javaStatus(input,destination,{connectImpl=createConnection}={}){
  return new Promise((resolve,reject)=>{
    ...
    function finish(err,result){if(done)return;done=true;clearTimeout(timer);socket.destroy();err?reject(err):resolve(result);}
...
async function bedrockStatus(input,destination){return new Promise((resolve,reject)=>{const socket=createSocket(destination.family===6?"udp6":"udp4"),nonce=randomBytes(8);let done=false;const timer=setTimeout(()=>finish(error("The Minecraft Bedrock status check timed out.")),5500);function finish(e,v){if(done)return;done=true;clearTimeout(timer);try{socket.close();}catch{}e?reject(e):resolve(v);}socket.once("error",()=>finish(error("The Minecraft Bedrock server did not answer its status check.")));socket.on("message",(bytes,remote)=>{if(remote.address!==destination.ip||remote.port!==destination.port)return;try{if(bytes.length>4096)throw error("Oversized Bedrock response.");finish(null,parseBedrockPong(bytes,nonce));}catch(e){finish(e);}});socket.send(Buffer.concat([Buffer.from([1]),nonce,MAGIC,randomBytes(8)]),destination.port,destination.ip,e=>{if(e)finish(error("The Minecraft Bedrock status check failed."));});});}
export async function fetchMinecraftServer(value,{edition="java",destinationImpl=minecraftDestination,javaImpl=javaStatus,bedrockImpl=bedrockStatus}={}){
  const input=parseMinecraftAddress(value,edition);const destination=await destinationImpl(input);
  return normalizeMinecraftStatus(await(edition==="java"?javaImpl(input,destination):bedrockImpl(input,destination)),input);
}
